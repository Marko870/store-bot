...
class Database:
    ...
    def get_user_by_username(self, username: str):
        """بحث بالـ username بدون @ — case insensitive"""
    ...
    def extend_subscription(self, sub_id, days):
        # FIX 6: INTERVAL لا تدعم parameterized مباشرة — نستخدم make_interval
        self.execute("""
    ...
    def get_exchange_orders(self, status=None, page=0, per_page=5, search=None):
        """طلبات الصرافة مع pagination وبحث"""
        offset = page * per_page
        conditions = []
        params = []
        if status and status != "all":
            conditions.append("co.status = %s")
            params.append(status)
        if search:
            conditions.append("(u.full_name ILIKE %s OR u.username ILIKE %s OR CAST(co.id AS TEXT) = %s)")
            params += [f"%{search}%", f"%{search}%", search]
        where = ("WHERE " + " AND ".join(conditions)) if conditions else ""
        rows = self.fetch(f"""
            SELECT co.*, u.full_name, u.username,
                   co.amount_usdt as amount,
                   co.status as status
            FROM currency_orders co
            JOIN users u ON co.user_id = u.id
            {where}
        ...
        """, params + [per_page, offset])
        total_row = self.fetchone(f"""
            SELECT COUNT(*) as c FROM currency_orders co
            JOIN users u ON co.user_id = u.id
            {where}
        """, params)
        total = total_row["c"] if total_row else 0
