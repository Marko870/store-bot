...
def make_fake_query(update):
    """دالة مساعدة مشتركة — تحاكي CallbackQuery من Message"""
    class _FakeQuery:
...
def parse_options(text: str) -> tuple[list, list]:
    """
    يحلل نص الخيارات ويرجع (options_list, added_labels).
    صيغة الإدخال:
        السؤال: نص السؤال؟
        خيار1: نص الخيار
        خيار2: نص آخر
...
(PLAN_SVC, PLAN_NAME_AR, PLAN_NAME_EN, PLAN_DAYS, PLAN_PRICE, PLAN_PRICE_SYP, PLAN_FEATS, PLAN_OPTS_Q) = range(20, 28)
...
async def svc_min(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        return SVC_MIN
    w = context.user_data.get("wiz", {})
    svc_id = db.add_service(w["name_ar"], w["name_en"], w.get("desc", ""), "", "digital", w["type_id"], min_val)
    context.user_data.pop("wiz", None)
    await update.message.reply_text(
        f"✅ *تم إضافة الخدمة!*\n\n"
        f"🔵 *{w['name_ar']}* (ID: {svc_id})\n\n"
        f"الخطوة التالية:\n"
        f"• أضف *أنواعاً* لها من 🗂️ أنواع الاشتراكات\n"
        f"• أو أضف *خطة مباشرة* من ➕ خطة جديدة",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("➕ أضف خطة الآن",        callback_data="wiz_add_plan")],
            [InlineKeyboardButton("🗂️ إدارة الأنواع",       callback_data="wiz_variants")],
...
async def wiz_add_plan_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(q.from_user.id): return ConversationHandler.END
    services = db.get_services()
    # الخطط للاشتراكات الرقمية فقط — التعبئة والصرافة لهم إعدادات منفصلة
    sub_services = [s for s in (services or []) if s.get("type_name") == "subscription"]
    if not sub_services:
        await q.edit_message_text(
            "❌ لا توجد خدمات اشتراكات رقمية\n\n"
            "الخطط تخص الاشتراكات فقط.\n"
            "التعبئة والصرافة لهم إعدادات منفصلة من لوحة الأدمن.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ أضف خدمة اشتراك", callback_data="wiz_add_svc")],
                [InlineKeyboardButton("◀️ رجوع", callback_data="admin_back")],
            ]))
        return ConversationHandler.END
    context.user_data["wiz"] = {}
    btns = [[InlineKeyboardButton(f"📺 {s['name_ar']}", callback_data=f"plansvc_{s['id']}")] for s in sub_services]
    btns.append([InlineKeyboardButton("◀️ رجوع", callback_data="admin_back"),
                 InlineKeyboardButton("❌ إلغاء", callback_data="wizard_cancel")])
    await q.edit_message_text(
        "➕ *خطة جديدة*\n\nاختر خدمة الاشتراك:",
...
async def plan_days_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if wiz.get("_custom_days"):
        try:
            days = int(update.message.text.strip())
            if days < 0:
                raise ValueError("negative days")
            wiz["days"] = days
            wiz.pop("_custom_days", None)
        except (ValueError, TypeError):
            await update.message.reply_text("❌ أرسل رقماً صحيحاً (مثال: 30):")
            return PLAN_DAYS
        await update.message.reply_text(
            "أرسل *السعر بالـ USDT*:\nمثال: 5.99",
            parse_mode="Markdown", reply_markup=cancel_kb())
        return PLAN_PRICE
    # الأدمن أرسل نص في وقت غلط — تذكيره
    await update.message.reply_text(
        "اختر المدة من الأزرار أعلاه، أو اضغط ✏️ للإدخال اليدوي.")
    return PLAN_DAYS
...
async def plan_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if price <= 0:
            raise ValueError
        context.user_data["wiz"]["price"] = price
    except Exception:
        await update.message.reply_text("❌ أرسل رقماً صحيحاً مثل: 5.99")
        return PLAN_PRICE
    await update.message.reply_text(
        "أرسل *السعر بالليرة السورية* (ل.س):\n"
        "مثال: 150000\n\n"
        "أو `-` لعدم عرض سعر الليرة:",
        parse_mode="Markdown", reply_markup=back_cancel_kb("wiz_add_plan"))
    return PLAN_PRICE_SYP


async def plan_price_syp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.strip().replace(",", "")
    if text == "-":
        context.user_data["wiz"]["price_syp"] = 0
    else:
        try:
            syp = float(text)
            if syp < 0:
                raise ValueError
            context.user_data["wiz"]["price_syp"] = syp
        except Exception:
            await update.message.reply_text("❌ أرسل رقماً صحيحاً مثل: 150000 أو `-` لتخطي")
            return PLAN_PRICE_SYP
    await update.message.reply_text(
...
async def plan_collect_opts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # نستخدم المرجع المباشر لـ context.user_data["wiz"]
    if "wiz" not in context.user_data:
        await update.message.reply_text("❌ انتهت الجلسة، أعد المحاولة من /admin")
        return ConversationHandler.END

    wiz = context.user_data["wiz"]

    if text == "تم":
        opts = wiz.get("options", [])
        await _save_plan_msg(update, context, opts)
        return ConversationHandler.END

    new_opts, added = parse_options(text)
    if added:
        if "options" not in wiz:
            wiz["options"] = []
        wiz["options"].extend(new_opts)
...
async def _save_plan(update_or_q, context, options, is_callback=True):
    """دالة موحدة لحفظ الخطة — تعمل مع callback query أو message"""
    w = context.user_data.get("wiz", {})
    db.add_plan_full(
        ...
        price_syp=w.get("price_syp", 0)
    )
    context.user_data.pop("wiz", None)
    svc = db.get_service(w["svc_id"])
    syp_line = f"\n💵 {w.get('price_syp', 0):,.0f} ل.س" if w.get("price_syp") else ""
    text = (
        f"✅ *تم إضافة الخطة!*\n\n"
        f"🔵 {svc['name_ar']}\n"
        f"📦 {w['name_ar']} — {w['price']} USDT{syp_line}\n"
        f"⏱ المدة: {w.get('days', 0)} يوم\n"
    ...
    if is_callback:
        await update_or_q.edit_message_text(text, parse_mode="Markdown", reply_markup=kb)
    else:
        await update_or_q.message.reply_text(text, parse_mode="Markdown", reply_markup=kb)


async def _save_plan_msg(update, context, options):
    """للتوافق مع الكود القديم"""
    await _save_plan(update, context, options, is_callback=False)
...
async def edit_svc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = "🟢 ظاهرة" if svc.get("is_active", 1) else "🔴 مخفية"
    info = (
        f"✏️ *{svc['name_ar']}*  |  {svc.get('type_label_ar', '—')}\n"
        f"📋 الحالة: {status}\n"
        f"📝 الوصف: {svc.get('description_ar') or '—'}\n"
        f"📊 الحد الأدنى: {svc.get('min_amount', 0)} USDT\n\n"
        f"اختر ما تريد تعديله:"
    )

    btns = [
        [InlineKeyboardButton("✏️ الاسم العربي",    callback_data="ef_name_ar"),
         InlineKeyboardButton("✏️ الاسم الإنجليزي", callback_data="ef_name_en")],
        [InlineKeyboardButton("✏️ الوصف",           callback_data="ef_description_ar"),
         InlineKeyboardButton("✏️ الحد الأدنى",     callback_data="ef_min_amount")],
    ]
    # زر الإخفاء/الإظهار حسب الحالة الحالية
    if svc.get("is_active", 1):
        btns.append([InlineKeyboardButton("🔴 إخفاء الخدمة", callback_data="ef_hide")])
    else:
        btns.append([InlineKeyboardButton("🟢 إظهار الخدمة", callback_data="ef_show")])

    # خيارات الخطط منفصلة بوضوح
    if plans:
        btns.append([InlineKeyboardButton(f"── الخطط ({len(plans)}) ──", callback_data="noop")])
        for p in plans:
            syp = p.get("price_syp", 0) or 0
            syp_str = f" | {syp:,.0f} ل.س" if syp > 0 else ""
            # سطر معلومات الخطة
            btns.append([InlineKeyboardButton(
                f"📦 {p['name_ar']} — {p['price']} USDT{syp_str}",
                callback_data="noop"
            )])
            # أزرار التعديل لهذه الخطة
            btns.append([
                InlineKeyboardButton("💲 سعر USDT",  callback_data=f"ef_planusdt_{p['id']}"),
                InlineKeyboardButton("💵 سعر ل.س",   callback_data=f"ef_plansyp_{p['id']}"),
                InlineKeyboardButton("🔘 خيارات",    callback_data=f"ef_planopts_{p['id']}"),
            ])

    btns.append([InlineKeyboardButton("◀️ رجوع", callback_data="wiz_edit_svc"),
                 InlineKeyboardButton("❌ إلغاء", callback_data="wizard_cancel")])
    await q.edit_message_text(info, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(btns))
    return EDIT_FIELD
...
async def edit_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تعديل سعر USDT لخطة محددة
    if field.startswith("planusdt_"):
        plan_id = int(field.replace("planusdt_", ""))
        context.user_data["wiz"]["edit_plan_usdt_id"] = plan_id
        plan = db.get_plan(plan_id)
        current = plan.get("price", 0) if plan else 0
        await q.edit_message_text(
            f"💲 *تعديل سعر USDT*\n\n"
            f"الخطة: *{plan['name_ar'] if plan else '—'}*\n"
            f"السعر الحالي: `{current}` USDT\n\n"
            "أرسل السعر الجديد بالـ USDT:\nمثال: 5.99",
            parse_mode="Markdown", reply_markup=cancel_kb())
        return EDIT_VALUE

    # تعديل سعر الليرة لخطة محددة
    if field.startswith("plansyp_"):
        plan_id = int(field.replace("plansyp_", ""))
        context.user_data["wiz"]["edit_plan_syp_id"] = plan_id
        from database import Database as _DB
        _db = _DB()
        plan = _db.get_plan(plan_id)
        current = plan.get("price_syp", 0) if plan else 0
        await q.edit_message_text(
            f"💵 *تعديل سعر الليرة*\n\n"
            f"الخطة: *{plan['name_ar'] if plan else '—'}*\n"
            f"السعر الحالي: `{current:,.0f}` ل.س\n\n"
            "أرسل السعر الجديد بالليرة أو `-` لإزالته:",
            parse_mode="Markdown", reply_markup=cancel_kb())
        return EDIT_VALUE
...
async def edit_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # حالة تعديل سعر USDT للخطة
    if w.get("edit_plan_usdt_id"):
        plan_id = w["edit_plan_usdt_id"]
        try:
            ...
            await update.message.reply_text("❌ أرسل رقماً صحيحاً مثل: 5.99")
            return EDIT_VALUE
        db.update_plan_field(plan_id, "price", usdt)
        context.user_data.pop("wiz", None)
        await update.message.reply_text(
            f"✅ تم تحديث السعر: *{usdt} USDT*",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("◀️ لوحة الأدمن", callback_data="admin_back")
            ]]))
    ...
    # حالة تعديل سعر الليرة للخطة
    if w.get("edit_plan_syp_id"):
        plan_id = w["edit_plan_syp_id"]
        if value == "-":
            syp = 0.0
        else:
            try:
                syp = float(value.replace(",", ""))
                if syp < 0:
                    raise ValueError
            except Exception:
                await update.message.reply_text("❌ أرسل رقماً صحيحاً أو `-` لإزالة السعر")
                return EDIT_VALUE
        db.update_plan_field(plan_id, "price_syp", syp)
        context.user_data.pop("wiz", None)
        syp_str = f"{syp:,.0f} ل.س" if syp > 0 else "لا يوجد"
        await update.message.reply_text(
            f"✅ تم تحديث سعر الليرة: *{syp_str}*",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("◀️ لوحة الأدمن", callback_data="admin_back")
            ]]))
...
async def edit_plan_opts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text    = update.message.text.strip()
    plan_id = context.user_data.get("wiz", {}).get("edit_plan_id")
    if not plan_id:
        await update.message.reply_text("❌ انتهت الجلسة، أعد المحاولة من /admin")
        return ConversationHandler.END
    ...
    options, added = parse_options(text)
    if not options:
...
async def mansub_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # FIX B: بحث في DB مباشرة بدل تحميل كل المستخدمين
...
async def cb_orders_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pending_exc = db.get_exchange_orders(status="pending")[1]
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(
            "💱 طلبات الصرافة" + (f" ({pending_exc} معلق)" if pending_exc else ""),
            callback_data="ordtype_exchange_pending_0")],
        [InlineKeyboardButton("◀️ رجوع", callback_data="admin_back")],
    ])
...
async def cb_orders_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    # ordtype_{order_type}_{status}_{page}
    # status ممكن يحتوي _ مثل awaiting_approval
    parts      = q.data.split("_")
    order_type = parts[1]
    page       = int(parts[-1])
    status     = "_".join(parts[2:-1])  # كل ما بين order_type وpage
...
async def cb_orders_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    # ordfilter_{order_type}_{status}
    parts      = q.data.split("_")
    order_type = parts[1]
...
async def _show_orders(q, context, order_type, status, page, search=None):
    ...
    if order_type == "recharge":
        ...
        orders, total = db.get_exchange_orders(status=status_param, page=page, per_page=PER_PAGE, search=search)
    else:
        orders, total = db.get_subscription_orders(status=status_param, page=page, per_page=PER_PAGE, search=search)

    type_label = {"recharge": "التعبئة", "exchange": "الصرافة"}.get(order_type, "الاشتراكات")
    if not orders:
        await q.edit_message_text(
            "لا توجد طلبات.",
    ...
    try:
        ...
        if opts:
            ...
            for k, v in opts.items():
                # نحط القيمة في code block لتجنب كسر Markdown
...
async def cb_order_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        # FIX: تحقق مرة أخرى من الحالة لمنع القبول المزدوج
        fresh = db.get_order_by_id(order_id)
        if not fresh or fresh.get("status") not in ("awaiting_approval", "pending"):
        ...
        if is_rch:
            ...
            # لا نرسل إشعار هنا — ننتظر إرسال البيانات من الأدمن
            pass
        user_id  = order["user_id"]
        plan_id  = order.get("plan_id") or 0
        plan_obj = db.get_plan(plan_id) if plan_id else None
        ...
        creds_data = {
            ...
            "order_id":  order_id,
            "plan_id":   plan_id,
            "plan_name": plan_obj["name_ar"] if plan_obj else order.get("plan_name_ar", "—"),
            "svc_name":  order.get("svc_ar", "—"),
        }
        from handlers import db as _hdb
        _hdb.set_user_state(q.from_user.id, f"SENDING_CREDS:{_j.dumps(creds_data)}")

        await q.edit_message_text(
            f"✅ تم قبول الطلب #{order_id}\nأرسل بيانات الدخول للزبون:",
            reply_markup=InlineKeyboardMarkup([
...
async def broadcast_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, u in enumerate(users):
        try:
            ...
            fail += 1
        # Rate limiting: توقف 35ms بين كل رسالة لتجنب حظر Telegram
        if i % 25 == 0 and i > 0:
            await asyncio.sleep(1)
    await update.message.reply_text(f"📢 اكتمل الإرسال: ✅ {ok} نجح | ❌ {fail} فشل")
    return ConversationHandler.END
...
async def var_svc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if variants:
        for v in variants:
            ...
            plans = db.get_plans_by_variant(v["id"])
            lines.append(
                f"• *{v['name_ar']}* — {len(opts)} خيارات | {len(plans)} خطة"
            )
            # زر حذف فقط — بدون زر تعديل لأن التعديل يتم عبر edit_svc
            btns.append([InlineKeyboardButton(
                f"🗑️ حذف: {v['name_ar']}",
...
async def var_collect_opts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "تم":
        svc      = db.get_service(w["svc_id"])
        var_opts = w.get("var_options", [])
        db.add_variant(w["svc_id"], w["var_name_ar"], w["var_name_en"], options=var_opts)
        context.user_data.pop("wiz", None)
        await update.message.reply_text(
            f"✅ تم إضافة النوع *{w['var_name_ar']}* بـ {len(var_opts)} خيارات\n\n"
            f"الآن أضف خططاً له من /admin ← ➕ خطة جديدة",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("◀️ لوحة الأدمن", callback_data="admin_back")
    ...
    new_opts, added = parse_options(text)
    if added:
        if "var_options" not in w:
            w["var_options"] = []
        w["var_options"].extend(new_opts)
        await update.message.reply_text(
            "\n".join(f"✅ {a}" for a in added) + "\n\nأرسل خياراً آخر أو `تم`",
            parse_mode="Markdown")
    else:
    ...
    return ORDER_HANDLERS + [
        ...
        ConversationHandler(
            ...
            states={
                ...
                PLAN_PRICE_SYP: [MessageHandler(filters.TEXT & ~filters.COMMAND, plan_price_syp)],
        ...
        ConversationHandler(
            ...
            states={
                # RCH_SVC و RCH_RATE يشتركان بنفس الـ callbacks — دُمجا لتجنب التكرار
                RCH_SVC: [
