...
async def edit_svc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if plans:
        ...
        for p in plans:
            opts_count = len(db.get_plan_options(p["id"]))
            syp_str = f" | {p.get('price_syp', 0):,.0f} ل.س" if p.get("price_syp") else ""
            btns.append([InlineKeyboardButton(
                f"🔘 {p['name_ar']} — {p['price']} USDT{syp_str}",
                callback_data=f"ef_planopts_{p['id']}"
            )])
            btns.append([InlineKeyboardButton(
                f"💵 تعديل سعر الليرة: {p['name_ar']}",
                callback_data=f"ef_plansyp_{p['id']}"
            )])
