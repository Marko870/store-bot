...
async def cb_exchange_method(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX 8: توضيح للحوالة إن سعرها = normal
    rate_note = "\n_ملاحظة: سعر الحوالة هو نفس السعر العادي_" if method == "hawala" else ""

    text = (
        f"💱 *{op_label}* — {method_label}\n\n"
        f"💵 السعر: `{rate:,.0f}` ل.س لكل USDT\n"
        f"📌 الحد الأدنى: `5 USDT`{rate_note}\n\n"
