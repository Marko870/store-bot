...
async def cb_services_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cat == "recharge":
        ...
        if len(recharge_svcs) == 1:
            ...
            svc = recharge_svcs[0]
            await cb_recharge_start(update, context, svc_id=svc['id'])
        else:
            # أكثر من خدمة — نعرضهم
...
async def handle_incoming(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ١ب. التحقق من رقم عملية سيرياتيل (نص)
...
async def verify_syriatel_tx(tx_no: str, expected_amount: float, gsm: str, api_key: str) -> dict:
    """التحقق من عملية سيرياتيل كاش عبر API Syria"""
    import aiohttp
    ...
    if not data.get("success"):
        return {"ok": False, "error": data.get("error", "خطأ غير معروف")}

    tx_data = data.get("data", {})
    if not tx_data.get("found"):
        return {"ok": False, "error": "رقم العملية غير موجود أو خارج نطاق 7 أيام"}
    ...
    if actual_amount < expected_amount:
        return {
            "ok": False,
            "error": f"المبلغ غير مطابق — المطلوب: {expected_amount:,.0f} ل.س، المُرسل: {actual_amount:,.0f} ل.س"
        }

    return {"ok": True, "tx": tx, "amount": actual_amount}
...
async def handle_recharge_proof(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    # يجب أن يكون نصاً (رقم العملية)
    if not update.message.text:
        await update.message.reply_text("أرسل *رقم العملية* من تطبيق سيرياتيل كاش.", parse_mode="Markdown")
        return True

    tx_no = update.message.text.strip()
    if not tx_no.isdigit():
        await update.message.reply_text("❌ رقم العملية يجب أن يكون أرقاماً فقط.\nمثال: `123456789`", parse_mode="Markdown")
        return True

    flow     = load_flow(uid)
    order_id = flow.get("order_id")
    amount   = flow.get("amount", 0)  # المبلغ بالليرة
    ...
    # تحقق من الـ API
    await update.message.reply_text("⏳ جاري التحقق من العملية...")

    result = await verify_syriatel_tx(
        tx_no=tx_no,
        expected_amount=amount,
        gsm=cfg.SYRIATEL_GSM,
        api_key=cfg.APISYRIA_KEY
    )

    if not result["ok"]:
        await update.message.reply_text(
            f"❌ *فشل التحقق*\n\n{result['error']}\n\nتحقق من رقم العملية وأعد المحاولة.",
            parse_mode="Markdown"
        )
        return True  # نبقى بنفس الـ state

    # التحقق نجح
    db.execute(
        "UPDATE orders SET status='paid', paid_at=NOW(), user_inputs=%s WHERE id=%s",
        (json.dumps({"tx_no": tx_no, "amount_received": result["amount"]}), order_id)
    )
    db.clear_user_state(uid)
    clear_flow(uid)
    ...
    if order:
        admin_text = (
            f"🔔 *طلب تعبئة #{order_id}* ✅ تم التحقق تلقائياً\n\n"
        ...
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("✅ تم التنفيذ", callback_data=f"rchdone_{order_id}_{uid}"),
            InlineKeyboardButton("❌ رفض",        callback_data=f"rchreject_{order_id}_{uid}")
        ]])
        for admin_id in cfg.ADMIN_IDS:
            try:
                await context.bot.send_message(
                ...
                logger.error(f"Recharge notify failed: {e}")
...
async def cb_recharge_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        "✅ *تم إنشاء الطلب*\n\n"
        f"حوّل `{amount:,.0f}` ل.س إلى رقم سيرياتيل كاش الخاص بنا:\n"
        f"`{cfg.SYRIATEL_GSM}`\n\n"
        "بعد التحويل، أرسل *رقم العملية* من تطبيق سيرياتيل كاش.\n"
        "_(الرقم يظهر في سجل العمليات)_",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
