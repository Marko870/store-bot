...
async def cb_service_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if variants:
        ...
        for p in plans:
            ...
            price_str = f"{p['price']} USDT"
            if syp > 0:
                price_str += f" | {syp:,.0f} ل.س"
            btns.append([InlineKeyboardButton(f"📦 {pname} — {price_str}",
                callback_data="plan_" + str(p["id"]))])
...
async def cb_variant_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in plans:
        ...
        if syp > 0:
            price_str += f" | {syp:,.0f} ل.س"
        btns.append([InlineKeyboardButton(f"📦 {pname} — {price_str}", callback_data=f"plan_{p['id']}")])
