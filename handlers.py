...
async def cb_send_creds(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """الأدمن يضغط 'إرسال بيانات' من المجموعة"""
    q = update.callback_query; await q.answer()
    ...
    # نحفظ في DB حتى يشتغل من المجموعة والخاص
    import json as _json
    db.set_user_state(admin_id, f"SENDING_CREDS:{_json.dumps({'user_id': user_id, 'order_id': order_id, 'plan_id': plan_id})}")
    ...
    # في المجموعة — نرد على الرسالة مباشرة
    try:
        await q.message.reply_text(
            f"✏️ @{q.from_user.username or admin_id} أرسل البيانات للزبون\n"
        ...
        logger.error(f"Send creds failed: {e}")
    ...
    try:
        ...
        if dest == "main_menu":
            ...
            if plans:
                ...
                text = f"🔵 *{svc['name_ar']}*\n🗂️ *{var['name_ar']}*"
                btns = [[InlineKeyboardButton(f"📦 {p['name_ar']} — {p['price']} USDT", callback_data=f"plan_{p['id']}")] for p in plans]
...
async def handle_exchange_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if state == EXCHANGE_AMOUNT:
        ...
        if not rate or rate <= 0:
            await update.message.reply_text(
                "❌ سعر الصرف غير محدد، تواصل مع الدعم."
            )
            db.clear_user_state(uid)
            return True
