...
async def cb_admin_skip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # نبعت رسالة تخطي بدل مسح الأزرار — الطلب يبقى موجوداً
    try:
        await q.message.reply_text("⏭️ تم التخطي — الطلب لا يزال معلقاً في قائمة الطلبات.")
    except Exception:
        await q.answer("⏭️ تم التخطي", show_alert=False)
